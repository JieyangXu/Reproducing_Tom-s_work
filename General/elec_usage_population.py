...
def upload_year(year):
        if year != '2020':
                ...
                year_r = year
        data = pd.read_excel('Data/LSOA_domestic_elec_2010-20.xlsx',sheet_name=year_r,skiprows=4)
        LSOA_codes = data['Lower layer super output area'].values
        met_num = data['Number of meters'].values
        consump = data["Total consumption(kWh)"].values
        ...
        for g in tqdm(range(len(len_query)-1)):
                ...
                compa:%s rdf:type comp:Electricity;
                        comp:hasStartUTC "%s"^^xsd:dateTime ;
                        comp:hasEndUTC "%s"^^xsd:dateTime .
                
                ons:%s rdf:type ons_t:Statistical-Geography.
                ons:%s comp:hasConsumed compa:%s.

                compa:%s rdf:type om:Energy;
                        om:hasPhenomenon compa:%s;
                        om:hasValue compa:%s.
                compa:%s om:hasNumericalValue compa:%s.
                compa:%s rdf:type gas:ElecMeters.
                ons:%s gas:hasElecMeters compa:%s.
                compa:%s gas:hasConsumingElecMeters compa:%s;
                        comp:hasStartUTC "%s"^^xsd:dateTime;
                        comp:hasEndUTC "%s"^^xsd:dateTime. 
                '''%(mes_uuid,
                mes_uuid, 
