...
namespace = 'http://localhost:8080/blazegraph/namespace/ontogasgrid/sparql'
...
def region_usage_query(limit):
    '''
    Querying the KG for all regions gas-usages in 2020 
    '''
    query='''
    PREFIX rdf:     <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
    PREFIX ons_t:    <http://statistics.data.gov.uk/def/statistical-geography#>
...
def region_elec_usage_query(limit):
    '''
    Querying the KG for all regions elec-usages in 2020 
    '''
    query='''
    PREFIX rdf:     <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
    PREFIX ons_t:    <http://statistics.data.gov.uk/def/statistical-geography#>
...
def region_meters_query(limit):
    '''
    Querying the KG for all regions gas meters in 2020 
    '''
    query='''
    PREFIX rdf:     <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
    PREFIX ons_t:    <http://statistics.data.gov.uk/def/statistical-geography#>
...
def region_elec_meters_query(limit):
    '''
    Querying the KG for all regions electricity meters in 2020 
    '''
    query='''
    PREFIX rdf:     <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
    PREFIX ons_t:    <http://statistics.data.gov.uk/def/statistical-geography#>
...
def region_fuel_pov_query(limit):
    '''
    Querying the KG for all regions fuel poverty in 2020 
...
testing = False # True for pickle, False for query
...
filename = 'C:/Users/Bro Soup/Desktop/University of Cambridge/Mphil in ChemE&Biotech/Heat_pump_code/Temporary/Code/pickle_files/temp_array'
gas_filename = 'C:/Users/Bro Soup/Desktop/University of Cambridge/Mphil in ChemE&Biotech/Heat_pump_code/Temporary/Code/pickle_files/gas_array'
meters_filename = 'C:/Users/Bro Soup/Desktop/University of Cambridge/Mphil in ChemE&Biotech/Heat_pump_code/Temporary/Code/pickle_files/meters_array'
elec_filename = 'C:/Users/Bro Soup/Desktop/University of Cambridge/Mphil in ChemE&Biotech/Heat_pump_code/Temporary/Code/pickle_files/elec_array'
elec_meters_filename = 'C:/Users/Bro Soup/Desktop/University of Cambridge/Mphil in ChemE&Biotech/Heat_pump_code/Temporary/Code/pickle_files/elec_meters_array'
fuel_poor_filename = 'C:/Users/Bro Soup/Desktop/University of Cambridge/Mphil in ChemE&Biotech/Heat_pump_code/Temporary/Code/pickle_files/fuel_poor'
if testing == True: 
...
# vector of TOTAL gas consumption in 2020 by month
...
# vector of TOTAL electricity consumption in 2020 by month
# used to scale yearly LSOA values
# ASSUMPTION: all LSOAs scale proportionately to yearly demand.
...
def dataframe_construction(month,uptake,temp_var_type,complete_df):
    ...
    new_df['Month (2020)'] = list(np.array([month_str for i in range(len(gas_values))]))
...
def plot_box(var):
    ...
    plt.tight_layout()
    ax_box = sb.boxplot(y=complete_df['Gas (kWh)'],x=complete_df['Month (2020)'],fliersize=0.05,whis=2,linewidth=1.2,ax=axs[0],color='w',flierprops=flierprops)
    sb.pointplot(x=complete_df['Month (2020)'], y=complete_df['Gas (kWh)'], data=complete_df.groupby('Gas (kWh)', as_index=False).median(), ax=axs[0],color='r',markers='.')
    ...
    med_gas = np.median(complete_df['Gas (kWh)'].values)
    left,right = axs[0].get_xlim()
    axs[0].set_xlabel('')
    axs[0].set_ylabel('Gas Consumption \n (kWh/month/household)')

    ax_box = sb.boxplot(y=complete_df['Electricity (kWh)'],x=complete_df['Month (2020)'],fliersize=0.05,whis=2,linewidth=1.2,ax = axs[1],color='w',flierprops=flierprops)
    sb.pointplot(x=complete_df['Month (2020)'], y=complete_df['Electricity (kWh)'], data=complete_df.groupby('Electricity (kWh)', as_index=False).median(), ax=axs[1],color='r',markers='.')
    ...
    for i,box in enumerate(ax_box.artists):
        ...
        for j in range(6*i,6*(i+1)):
            ax_box.lines[j].set_color('black')
    med_elec = np.median(complete_df['Electricity (kWh)'].values)
    left,right = axs[1].get_xlim()
    plt.subplots_adjust(left = 0.175,bottom=0.092,right=0.967,top=0.949,wspace=0.2,hspace=0.14)
    axs[1].set_xticklabels(months_letter)
   ...
   #axs[0].set_yticks(np.array([0.0,1.0E6,2.0E6,3.0E6,4.0E6]))
    plt.legend()
    plt.savefig('C:/Users/Bro Soup/Desktop/University of Cambridge/Mphil in ChemE&Biotech/Heat_pump_code/Temporary/Code/figure_output/box_and_whisker.png',dpi=200)
    plt.savefig('C:/Users/Bro Soup/Desktop/University of Cambridge/Mphil in ChemE&Biotech/Heat_pump_code/Temporary/Code/figure_output/box_and_whisker.pdf')
